...
document.getElementById("buttonnsmb").onclick = async () => {
  document.getElementById("buttonnsmb").style.display = "none";
  document.getElementById("romload").style.display = "none";
  console.log(document.getElementById("romload").value, "what")
    // YOUR FILE NAME GOES HERE!!!!!!
    var file = await fetch("/" + document.getElementById("romload").value + ".nds").then(r => r.blob());
    if (!file) {
        return
    }
  tryLoadROM(file)
